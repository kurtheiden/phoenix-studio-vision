...
pub(crate) struct Inspection {
    ...
    pub(crate) discovery: DiscoveryAnalysis,
}

#[derive(Debug, PartialEq, Eq)]
pub(crate) struct PrintableString {
...
pub(crate) struct DiscoveryAnalysis {
    pub(crate) strings: Vec<PrintableString>,
    pub(crate) printable_bytes: u64,
    total_bytes: u64,
    pub(crate) entropy: f64,
}

impl DiscoveryAnalysis {
    pub(crate) fn longest_string(&self) -> Option<&PrintableString> {
    ...
    pub(crate) fn printable_percentage(&self) -> f64 {
...
pub(crate) fn inspect(path: &Path) -> Result<Inspection, Box<dyn Error>> {
...
mod tests {
    ...
    fn hex_dump_formats_hex_ascii_and_padding() {
        assert_eq!(
            format_hex_dump(b"Phoenix\0\xff"),
    ...
    fn hex_dump_uses_sixteen_byte_rows() {
        let dump = format_hex_dump(&(0..17).collect::<Vec<u8>>());
    ...
    fn printable_strings_require_four_characters() {
        assert_eq!(
            analyze_bytes(b"abc\0abcd\0~~~~").strings,
            vec![
                PrintableString {
                    offset: 4,
                    value: "abcd".into()
    ...
    fn printable_string_offsets_cover_adjacent_regions_in_file_order() {
        assert_eq!(
            analyze_bytes(b"\0First\x1fSecond\x7fLast\0").strings,
            vec![
                PrintableString {
                    offset: 1,
                    value: "First".into()
    ...
    fn longest_string_is_the_first_longest_in_file_order() {
        ...
        let longest = analysis.longest_string().unwrap();
        assert_eq!((longest.offset, longest.value.as_str()), (5, "longest"));
    }
    ...
    fn entropy_matches_simple_known_distributions() {
        assert!(analyze_bytes(&[0; 8]).entropy.abs() < f64::EPSILON);
        assert!((analyze_bytes(&[0, 1, 0, 1]).entropy - 1.0).abs() < f64::EPSILON);
        assert!((analyze_bytes(&[0, 1, 2, 3]).entropy - 2.0).abs() < f64::EPSILON);
    }
}
